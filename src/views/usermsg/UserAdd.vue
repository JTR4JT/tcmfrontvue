<template>
    <el-form ref="form" :rules="rules" :model="user" label-width="80px" style="width: 600px">
        ...
        <el-form-item label="账号" prop="uaccount" readonly>
    ...
    methods: {
        onSubmit(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    let _this = this
                    axios.put('http://localhost:8081/user/addUser', this.user).then(function (response) {
                        if (response.data) {
                            _this.$alert(_this.user.uname + '添加成功！', '添加数据', {
